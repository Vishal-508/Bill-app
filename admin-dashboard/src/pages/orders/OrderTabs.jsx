...
const TABS = ['items', 'bills', 'payments', 'timeline'];

export default function OrderTabs({ order }) {
  ...
  const paymentsCount = Array.isArray(order?.payments) ? order.payments.length : 0;
  const historyCount = Array.isArray(order?.statusHistory) ? order.statusHistory.length : 0;

  const tabs = [
    { label: `Items${itemCount ? ` (${itemCount})` : ''}`,
      render: () => <ItemsTab order={order} /> },
    { label: 'Bills',
      render: () => <BillsTab order={order} /> },
    { label: `Payments${paymentsCount ? ` (${paymentsCount})` : ''}`,
      render: () => <PaymentsTab order={order} /> },
    { label: `Timeline${historyCount ? ` (${historyCount})` : ''}`,
      render: () => <TimelineTab order={order} /> },
  ];
